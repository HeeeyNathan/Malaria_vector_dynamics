...
Dispersion
# 187.2638... That is overdispersion!
...
T5 <- boxplot(EPoi ~ waterbody_type,
        ylab = "Pearson residuals",
        data = df)
#That is not really convincing.
...
# Zero inflation?
# Simulation study
I2 <- inla(vec_abund ~ eqr.std + ppt.std + tmax.std + doy.std + waterbody_type,
           family = "poisson",
           data = df,
...
X <- model.matrix(~eqr.std + ppt.std + tmax.std + doy.std + waterbody_type,
                  data = df)
...
for (i in 1: NSim){
  ...
  Ysim[,i]    <- rpois(n = nrow(df), lambda = mu.i[,i])
}
