...
SA <- as_tibble(read.csv("Data/Diptera_indices_wLandcover_and_ENV_edited.csv", h = T, sep = ",", stringsAsFactors = FALSE, check.names = FALSE)) |> 
  ...
  mutate(vec_abund_pa    = ifelse(vec_abund == 0, 0, 1), # create vector with presence-absence data
         ...
         fwaterbody_type = factor(waterbody_type), # make waterbody_type a factor
...
p_pos <- ggmap(glgmap) +
  ...
  theme(text = element_text(size = 15),
        ...
        legend.background = element_rect(fill = "#FAFBF3", color = NA),  # match legend background
        legend.key = element_rect(fill = "#FAFBF3", color = NA))         # match legend key background
...
MyX <- c("year", "eqr",
         "agri_logit", "arti_logit", "natu_logit", "water_logit", "wetland_logit",
         "ppt", "q", "tmax", "tmin", "ws")
Mydotplot(SA[, MyX])
...
boxplot(agri_logit ~ fyear,
        data = SA)

boxplot(natu_logit ~ fyear,
        data = SA)
...
#      artificial areas - human disturbance?
#      natural areas - no disturbance?
#      average maximum temperatures from 12 months before sampling?
...
SA$agriculture.std <- MyStd(SA$agri_logit)
SA$natural.std     <- MyStd(SA$natu_logit)
...
DIC.ZANB.mesh1.A = sum(tapply(ZANB.mesh1A$dic$local.dic, ZANB.mesh1A$dic$family, sum))
DIC.ZANB.mesh1.B = sum(tapply(ZANB.mesh1B$dic$local.dic, ZANB.mesh1B$dic$family, sum))
DIC.ZANB.mesh1.C = sum(tapply(ZANB.mesh1C$dic$local.dic, ZANB.mesh1C$dic$family, sum))

WAIC.ZANB.mesh1.A = sum(tapply(ZANB.mesh1A$waic$local.waic, ZANB.mesh1A$dic$family, sum))
WAIC.ZANB.mesh1.B = sum(tapply(ZANB.mesh1B$waic$local.waic, ZANB.mesh1B$dic$family, sum))
WAIC.ZANB.mesh1.C = sum(tapply(ZANB.mesh1C$waic$local.waic, ZANB.mesh1C$dic$family, sum))

Z <- matrix(nrow = 4, ncol = 2)
Z[,1] <- c(DIC.ZANB.mesh1, DIC.ZANB.mesh1.A, DIC.ZANB.mesh1.B, DIC.ZANB.mesh1.C)
Z[,2] <- c(WAIC.ZANB.mesh1, WAIC.ZANB.mesh1.A, WAIC.ZANB.mesh1.B, WAIC.ZANB.mesh1.C)
...
ZANB.mesh1$summary.hyper[1,"mean"]
HyperZap <- list(theta = list(initial = -10, fixed = TRUE),
                 size = list(initial = 0.4732127, fixed = TRUE))  # dont know what to do here :/
...
DIC.ZANB.mesh1 = sum(tapply(ZANB.mesh1$dic$local.dic, ZANB.mesh1$dic$family, sum))
DIC.ZANB.mesh1.1 = sum(tapply(ZANB.mesh1.1$dic$local.dic, ZANB.mesh1.1$dic$family, sum))
DIC.ZANB.mesh1.2 = sum(tapply(ZANB.mesh1.2$dic$local.dic, ZANB.mesh1.2$dic$family, sum))
DIC.ZANB.mesh1.3 = sum(tapply(ZANB.mesh1.3$dic$local.dic, ZANB.mesh1.3$dic$family, sum))
DIC.ZANB.mesh1.4 = sum(tapply(ZANB.mesh1.4$dic$local.dic, ZANB.mesh1.4$dic$family, sum))
DIC.ZANB.mesh1.5 = sum(tapply(ZANB.mesh1.5$dic$local.dic, ZANB.mesh1.5$dic$family, sum))
DIC.ZANB.mesh1.6 = sum(tapply(ZANB.mesh1.6$dic$local.dic, ZANB.mesh1.6$dic$family, sum))
DIC.ZANB.mesh1.7 = sum(tapply(ZANB.mesh1.7$dic$local.dic, ZANB.mesh1.7$dic$family, sum))
DIC.ZANB.mesh1.8 = sum(tapply(ZANB.mesh1.8$dic$local.dic, ZANB.mesh1.8$dic$family, sum))
DIC.ZANB.mesh1.9 = sum(tapply(ZANB.mesh1.9$dic$local.dic, ZANB.mesh1.9$dic$family, sum))
DIC.ZANB.mesh1.10 = sum(tapply(ZANB.mesh1.10$dic$local.dic, ZANB.mesh1.10$dic$family, sum))
DIC.ZANB.mesh1.11 = sum(tapply(ZANB.mesh1.11$dic$local.dic, ZANB.mesh1.11$dic$family, sum))
DIC.ZANB.mesh1.12 = sum(tapply(ZANB.mesh1.12$dic$local.dic, ZANB.mesh1.12$dic$family, sum))
DIC.ZANB.mesh1.13 = sum(tapply(ZANB.mesh1.13$dic$local.dic, ZANB.mesh1.13$dic$family, sum))
DIC.ZANB.mesh1.14 = sum(tapply(ZANB.mesh1.14$dic$local.dic, ZANB.mesh1.14$dic$family, sum))
...
# DIC
# Full model              8313.024
# eqr count part          8313.702
# agriculture count part  8325.731
# artificial count part   8314.383  <------ drop
# natural count part      8336.440
# ppt count part          8329.628
# tmax count part         8318.211
# ws count part           8312.241
# eqr binary part         8429.597
# agriculture binary part 8312.103
# artificial binary part  8311.660
# natural binary part     8312.239
# ppt binary part         8312.548
# tmax binary part        8311.612
...
# DIC
# Full model              8363.033
# eqr count part          8369.912
# agriculture count part  8367.066
# natural count part      8373.672
# ppt count part          8379.050
# tmax count part         8364.132
# ws count part           8361.816
# eqr binary part         8480.372
# agriculture binary part 8363.858
# artificial binary part  8362.129
# natural binary part     8363.131
# ppt binary part         8361.539
...
# DIC
# Full model              8361.229
# eqr count part          8368.135
# agriculture count part  8365.245
# natural count part      8372.070
# ppt count part          8376.875
# tmax count part         8362.447
# ws count part           8360.461
# eqr binary part         8479.133
# agriculture binary part 8360.648
# artificial binary part  8359.504 <----- drop
# natural binary part     8361.308
...
# DIC
# Full model              8360.034
# eqr count part          8366.795
# agriculture count part  8363.840
# natural count part      8369.985
# ppt count part          8376.887
# tmax count part         8360.508
# ws count part           8359.802
# eqr binary part         8479.472
# agriculture binary part 8359.118
# natural binary part     8360.433
...
# DIC
# Full model              8358.952
# eqr count part          8365.572
# agriculture count part  8363.302
# natural count part      8368.693
# ppt count part          8374.288
# tmax count part         8359.409
# ws count part           8357.514 <----- drop
# eqr binary part         8479.182
# agriculture binary part 8359.739
...
# DIC
# Full model              8356.883
# eqr count part          8364.120
# agriculture count part  8361.076
# natural count part      8368.075
# ppt count part          8373.524
# tmax count part         8359.196
# eqr binary part         8477.841
# agriculture binary part 8356.595 <---- drop
...
# DIC
# Full model             8356.804
# eqr count part         8363.618
# agriculture count part 8361.036
# natural count part     8367.813
# ppt count part         8373.140
# tmax count part        8357.983
# eqr binary part        8477.130
...
# DIC
# Full model             8356.414
# eqr count part         8362.852
# agriculture count part 8360.663
# natural count part     8367.177
# ppt count part         8372.552
# tmax count part        8358.212
...
# DIC
# Full model             8354.557
# eqr count part         8361.721
# agriculture count part 8359.097
# natural count part     8366.012
# ppt count part         8371.323
# tmax count part        8357.669
# eqr binary part        8473.822
...
Beta01  <- ZANB.mesh1$summary.fixed[8:14,"mean"]
k       <- ZANB.mesh1$summary.hyper[1,"mean"]
Xpos <- as.matrix(X[, c(1:3, 5:8),])
X01.1  <- as.matrix(X01[, c(1:3, 5:8)])
...
mu <- exp(Xpos %*% BetaPos + A1.m %*% wPos)
P0 <- (k / (mu + k))^k
Pi.self <- exp(X01.1 %*% Beta01 + A1.m %*% w01) / (1 + exp(X01.1 %*% Beta01 + A1.m %*% w01))
...
# p <- 2 * (8 + 2 + 2)
sum(EZANB^2) / (nrow(SA) - p)
# 0.04311422 That is.... not overdispersion! :')))
...
PlotField(field = wpm.ZANB.Pos, mesh = mesh1,
          xlim = range(mesh1$loc[,1]),
          ylim = range(mesh1$loc[,2]))
points(x = Loc[,1],
       y = Loc[,2],
       ...
       col = "black",
       pch = 16)
plot(Land1, col = "white", add = TRUE, border = "white")
plot(Lithuania.UTM, add = TRUE)
text(x = min(range(mesh1$loc[,1]) + 210000),  # x position in the middle of the x-range
     y = min(range(mesh1$loc[,2]) + 1),  # y position in the middle of the y-range
     labels = paste("Mean SD of spatial random field:", round(mean(ZANB.mesh1$summary.random$wpos$sd), 2)),
     cex = 1.5,
     col = "Black")
dev.off()
...
PlotField(field = wpm.ZANB.01, mesh = mesh1,
          xlim = range(mesh1$loc[,1]),
          ylim = range(mesh1$loc[,2]))
points(x = Loc[,1],
       y = Loc[,2],
...
text(x = min(range(mesh1$loc[,1]) + 210000),  # x position in the middle of the x-range
     y = min(range(mesh1$loc[,2]) + 1),  # y position in the middle of the y-range
     labels = paste("Mean SD of spatial random field:", round(mean(ZANB.mesh1$summary.random$w01$sd), 2)),
     cex = 1.5,
     col = "Black")
dev.off()
...
fixed_effects <- ZANB.mesh1$summary.fixed
print(fixed_effects)

# Extract random effects
random_effects <- ZANB.mesh1$summary.random
print(random_effects)

# Extract model hyperparameters
hyperparameters <- ZANB.mesh1$summary.hyperpar
print(hyperparameters)

# Extract DIC and WAIC (model fit metrics)
dic <- ZANB.mesh1$dic$dic
waic <- ZANB.mesh1$waic$waic
effective_params <- ZANB.mesh1$dic$p.eff  # Effective number of parameters from DIC
...
save_gt_table(
  ...
  filename = "C:/Users/natha/OneDrive/University of Johannesburg/Post-Doc/Nature Research Centre/How Changes in Bioversity Change Biodiversity/Seminars/October 2024/positive_fixed_effects.png"
)
...
save_gt_table(
  ...
  filename = "C:/Users/natha/OneDrive/University of Johannesburg/Post-Doc/Nature Research Centre/How Changes in Bioversity Change Biodiversity/Seminars/October 2024/binary_fixed_effects.png"
)

# Extract the Range and sigma_u from the model's hyperparameters
range_km <- ZANB.mesh1$summary.hyperpar[c(3, 5), "mean"] / 1000  # Convert range mean to kilometers
range_sd_km <- ZANB.mesh1$summary.hyperpar[c(3, 5), "sd"] / 1000  # Convert range sd to kilometers
range_lower_km <- ZANB.mesh1$summary.hyperpar[c(3, 5), "0.025quant"] / 1000  # Convert lower 95% CI to kilometers
range_upper_km <- ZANB.mesh1$summary.hyperpar[c(3, 5), "0.975quant"] / 1000  # Convert upper 95% CI to kilometers

# Extract sigma_u values (no conversion needed)
sigma_u <- ZANB.mesh1$summary.hyperpar[c(4, 6), "mean"]
sigma_u_sd <- ZANB.mesh1$summary.hyperpar[c(4, 6), "sd"]
sigma_u_lower <- ZANB.mesh1$summary.hyperpar[c(4, 6), "0.025quant"]
sigma_u_upper <- ZANB.mesh1$summary.hyperpar[c(4, 6), "0.975quant"]
...
save_gt_custom_hyper_table(
  ...
  filename = "C:/Users/natha/OneDrive/University of Johannesburg/Post-Doc/Nature Research Centre/How Changes in Bioversity Change Biodiversity/Seminars/October 2024/key_hyperparameters.png"
)
...
save_gt_diagnostic_table(
  ...
  filename = "C:/Users/natha/OneDrive/University of Johannesburg/Post-Doc/Nature Research Centre/How Changes in Bioversity Change Biodiversity/Seminars/October 2024/model_diagnostics.png"
)
